include <../third_party/gridfinity-rebuilt-openscad/gridfinity-rebuilt-utility.scad>
...
module cutCube() {
    translate([l_grid / 2, 0 - distancey / 2]) {
        cube([distancex / 2, distancey, h_base]);
    };
}
...
module gridfinityBaseplate(gridx, gridy, length, dix, diy, sp, sm, sh, fitx, fity) {
    ...
    difference() {
        ...
        mirror([0,0,1])
        rounded_rectangle(dx, dy, h_base+off, 0);
